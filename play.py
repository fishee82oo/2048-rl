"""Watch a PPO or DQN policy play one complete game in the terminal."""
import argparse
from pathlib import Path
import time
from utils.evaluation import load_policy, policy_action
...
def main():
    ...
    parser.add_argument("--strategy", "--corner-filter", action=argparse.BooleanOptionalAction, default=None,
                        help="Override the strategy filter setting saved in the checkpoint")
    parser.add_argument('--algorithm', choices=['auto','ppo','dqn'], default='auto')
    ...
    set_seed(args.seed)
    env = Game2048(args.seed)
    agent, _ = load_policy(args.checkpoint, args.algorithm)
    if args.strategy is not None:
        agent.strategy = args.strategy
    state, done, moves = env.reset(), False, 0
    display(env, "START")
    while not done:
        action = policy_action(agent, env, args.deterministic)
